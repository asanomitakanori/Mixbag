...
class ProportionLoss(nn.Module):
    def __init__(self, metric="ce", eps=1e-8, reduction='mean'):
        super().__init__()
        self.metric = metric
        self.eps = eps
...
#         self.metric = metric
#         self.eps = eps
#         self.posi_loss = ProportionLoss(metric, eps, reduction='none')
#         # self.nega_loss = ProportionLoss(metric, eps)

#     def forward(self, input, target):
#         posi_nega = (target[:, 0] != 1).long()
#         input[posi_nega==1, 0] = 0 # mask negative class
#         input /= input.sum(dim=1, keepdims=True) # normalize

#         loss = posi_nega * self.posi_loss(input[:, 1:], target[:, 1:])
#         loss = loss.mean()
