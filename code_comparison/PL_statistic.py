...
import logging
from train_statistic import train_net
...
def main(args):
    ...
    make_folder(args.output_path)
    args.output_path +=   f'ins_aug-statistic' + f'/fold/ins_augtype_{args.augmentation}-standard{args.standard_normal_value}-choice{args.choice}-ins{args.ins}-bags{args.bags}/'
    make_folder(args.output_path)   
    ...
    train_loss, val_loss, test_acc = train_net(args, model, optimizer, train_loader, val_loader, test_loader, loss_function_train, loss_function_val)
...
if __name__ == '__main__':
    ...
    for fold in range(5):
        ...
        parser.add_argument('--wrong_plp',
                            default=False, type=bool, help="how to create mixed-bag")
    ...
    if os.path.exists(to_abs_path('result/' + args.dataset + f'/proposed_aug_tloss.json')):
        with open(to_abs_path('result/' + args.dataset + f'/proposed_aug_tloss.json'), 'r') as f:
            tloss = json.load(f)
        with open(to_abs_path('result/' + args.dataset + f'/proposed_aug_vloss.json'), 'r') as f:
            vloss = json.load(f)
        with open(to_abs_path('result/' + args.dataset + f'/proposed_aug_mIoU.json'), 'r') as f:
            tmIoU = json.load(f)

    tloss[f'ins_augtype_{args.augmentation}-standard{args.standard_normal_value}-choice{args.choice}-ins{args.ins}-bags{args.bags}'] = train_loss
    vloss[f'ins_augtype_{args.augmentation}-standard{args.standard_normal_value}-choice{args.choice}-ins{args.ins}-bags{args.bags}'] = val_loss
    tmIoU[f'ins_augtype_{args.augmentation}-standard{args.standard_normal_value}-choice{args.choice}-ins{args.ins}-bags{args.bags}'] = np.mean(test_list)

    with open(to_abs_path('result/' + args.dataset + f'/proposed_aug_tloss.json'), 'w') as f:
        json.dump(tloss, f, indent=4)
    with open(to_abs_path('result/' + args.dataset + f'/proposed_aug_vloss.json'), 'w') as f:
        json.dump(vloss, f, indent=4)
    with open(to_abs_path('result/' + args.dataset + f'/proposed_aug_mIoU.json'), 'w') as f:
