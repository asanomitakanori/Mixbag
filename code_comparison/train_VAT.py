...
def train_net_statistic(args, model, optimizer, train_loader, val_loader, test_loader, loss_function_train, loss_function_val, consistency_criterion):
    fix_seed(args.seed)
    ...
    for epoch in range(args.num_epochs):
        ...
        for iteration, (data, label, lp, min_point, max_point) in enumerate(tqdm(train_loader, leave=False)):
            ...
            if args.statistic and args.correct_label == False and args.wrong_plp == False:
                ...
                loss = loss_function_train(pred_prop, lp)
...
def train_net(args, model, optimizer, train_loader, val_loader, test_loader, loss_function_train, loss_function_val, consistency_criterion):
    fix_seed(args.seed)
    ...
    for epoch in range(args.num_epochs):
        ...
        losses = []
        gt, pred = [], []
        for iteration, (data, label, lp) in enumerate(tqdm(train_loader, leave=False)):
            (b, n, c, w, h) = data.size()
            data = data.reshape(-1, c, w, h)
            label = label.reshape(-1)
            data, lp = data.to(args.device), lp.to(args.device)
