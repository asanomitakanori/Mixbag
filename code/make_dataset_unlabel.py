...
if __name__ == '__main__':
    ...
    x = f'data/{dataset}-unlabel-ins{num_instances}-bags{num_posi_bags}'
    ...
    for i in range(num_posi_bags):
        ...
        for c in range(num_classes):
            ...
            unlabel_bag.extend(train_data[used_index[c][cnt[c]+num_c:]])
            unlabel_label.extend(train_label[used_index[c][cnt[c]+num_c:]])
            cnt[c] += num_c
        print(cnt)
    ...
    bags, labels, unlabel_bag, unlabel_label = np.array(bags), np.array(labels), np.array(unlabel_bag), np.array(unlabel_label)

    np.save(f'{x}/bags', bags)
    np.save(f'{x}/labels', labels)
    np.save(f'{x}/unlabel_bags', unlabel_bag)
    np.save(f'{x}/unlabel_labels', unlabel_label)
