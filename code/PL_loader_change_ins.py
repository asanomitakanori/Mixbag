    ...
    for bags in [80, 120, 160, 200, 240]:
        ...
        for aug in [True, False]:
            for seed in range(5):
                ...
                # parser.add_argument('--dataset', default=f'cifar10/true-3class-ins{ins}-bags{bags}-change',
                #                     type=str, help='name of dataset')
                parser.add_argument('--dataset', default=f'cifar10/tyofuku-3class-ins{ins}-bags{bags}-multi1',
                                    type=str, help='name of dataset')
                ...
                tloss[f'seed{args.seed}-ins{ins}-bags{args.bags}-aug{aug}'] = train_loss
                vloss[f'seed{args.seed}-ins{ins}-bags{args.bags}-aug{aug}'] = val_loss
                tmIoU[f'seed{args.seed}-ins{ins}-bags{args.bags}-aug{aug}'] = test_mIoU

        with open(to_abs_path('result/' + args.dataset + '/mini_batch{args.mini_batch}-change_loader_tloss.json'), 'w') as f:
            json.dump(tloss, f, indent=4)
        with open(to_abs_path('result/' + args.dataset + '/mini_batch{args.mini_batch}-change_loader_vloss.json'), 'w') as f:
            json.dump(vloss, f, indent=4)
        with open(to_abs_path('result/' + args.dataset + '/mini_batch{args.mini_batch}-change_loader_mIoU.json'), 'w') as f:
