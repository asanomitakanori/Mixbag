...
def load_data_bags(args):  # Toy
    ...
    if args.augmentation:
        if args.wrong_plp:
            ...
            train_dataset = DatasetBag_randomchoice(
                                                        args=args, 
                                                        data=train_bags, 
                                                        label=train_labels, 
                                                        lp=train_lps, 
    ...
    def __getitem__(self, idx):
        ...
        if n >= 0.5:
            ...
            if self.choice == 'half':
                index_order1 = index_order1[0:index_order1.shape[0] // 2]
                index_order2 = index_order2[0:index_order2.shape[0] // 2]
                min_error, max_error = error_cover_area(lp, choice_lp, len(index_order1), len(index_order2), 0.68)
            elif self.choice == 'uniform':
                x = np.random.randint(1, data.shape[0])
                index_order1 = index_order1[0:x]
                index_order2 = index_order2[x:]
            ...
            lp = self.lp[idx]
            lp = torch.tensor(lp).float()
            min_error, max_error = torch.full((1, self.classes), 0).reshape(self.classes).float(), torch.full((1, self.classes), 0).reshape(self.classes).float()
            return data, label, lp, [min_error, max_error]
...
def error_cover_area(
    ...
    confidence_area = t * ( np.sqrt(proportion1*(1-proportion1) / sampling_num1) * np.sqrt(proportion2*(1-proportion2) / sampling_num2) )
