...
def create_bags_val(data, label, num_posi_bags, num_nega_bags, args):
    # make poroportion
...
def create_bags(data, label, num_posi_bags, num_nega_bags, args):
    ...
    if args.aug_bags_num == 2:
        print('a')
    for x in range(1, args.aug_bags_num):
        # if x ==1:
        bags_tmp = np.concatenate([split_bags_tmp[-x:], split_bags_tmp[:-x]])
        labels_tmp = np.concatenate([split_labels_tmp[-x:], split_labels_tmp[:-x]])

        bags_new, labels_new = bags_temp[:, bags_temp.shape[1]//2:], labels_temp[:, labels_temp.shape[1]//2:]
        bags_new = np.concatenate([bags_tmp, bags_new], axis=1)
        labels_new = np.concatenate([labels_tmp, labels_new], axis=1)
        ...
        proportion_N = np.concatenate([proportion_N, new_Proportion], axis=0)
...
def main(args):
    ...
    for i, (train_idx, val_idx) in enumerate(skf.split(data, label)):
        ...
        output_path = 'data/%s/tyofuku-%dclass-ins%d-bags%d-multi%d/%d/' % (
            args.dataset, args.num_classes, args.num_instances, args.train_num_posi_bags, args.aug_bags_num ,i)
    ...
    np.save('data/%s/tyofuku-%dclass-ins%d-bags%d-multi%d/test_data' %
            (args.dataset, args.num_classes, args.num_instances, args.train_num_posi_bags, args.aug_bags_num ), test_data)
    np.save('data/%s/tyofuku-%dclass-ins%d-bags%d-multi%d/test_label' %
            (args.dataset, args.num_classes, args.num_instances, args.train_num_posi_bags, args.aug_bags_num ), test_label)
...
if __name__ == '__main__':
    ...
    for i in [80, 120, 160, 200, 240]:
        for x in [1, 2, 3, 4]:
            ...
            parser.add_argument('--aug_bags_num', default=x, type=int)
