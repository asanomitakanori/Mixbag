...
def main(args):
    ...
    train_loss, val_loss, test_mIoU = train_net(args, model, optimizer, train_loader, val_loader, test_loader, loss_function_train, loss_function_val)
...
if __name__ == '__main__':
    ...
    if os.path.exists(to_abs_path('result/' + args.dataset + f'/proposed-tloss.json')):
        with open(to_abs_path('result/' + args.dataset + f'/proposed-tloss.json'), 'r') as f:
            tloss = json.load(f)
        with open(to_abs_path('result/' + args.dataset + f'/proposed-vloss.json'), 'r') as f:
            vloss = json.load(f)
        with open(to_abs_path('result/' + args.dataset + f'/proposed-mIoU.json'), 'r') as f:
            tmIoU = json.load(f)

    tloss[f'standard{args.standard_normal_value}-choice{args.choice}-ins{args.ins}-bags{args.bags}'] = train_loss
    vloss[f'standard{args.standard_normal_value}-choice{args.choice}-ins{args.ins}-bags{args.bags}'] = val_loss
    tmIoU[f'standard{args.standard_normal_value}-choice{args.choice}-ins{args.ins}-bags{args.bags}'] = np.mean(test_list)

    with open(to_abs_path('result/' + args.dataset + f'/proposed-tloss.json'), 'w') as f:
        json.dump(tloss, f, indent=4)
    with open(to_abs_path('result/' + args.dataset + f'/proposed-vloss.json'), 'w') as f:
        json.dump(vloss, f, indent=4)
    with open(to_abs_path('result/' + args.dataset + f'/proposed-mIoU.json'), 'w') as f:
